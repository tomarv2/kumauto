...
#########################################################################
def build_prometheus(config_yaml, user_input_env, monitoring_config_file_path, monitoring_rules_dir,
                     monitoring_static_file_dir, project_name, targets_to_monitor, monitoring_rules_sample_file,
                     monitoring_static_file_sample_file, modules, project_name_without_env):
    logger.info("=" * 75)
    ...
    logger.info("prometheus: file location: %s", prometheus_fileloc)
    logger.info("prometheus: rules dir: %s", prometheus_rules_dir)
    logger.info("prometheus: static_files dir: %s", prometheus_staticfiles_dir)
    logger.info("prometheus: verifying if config already exists...")
    ...
    try:
        if prometheus_validate_current_setup(prometheus_fileloc, project_name, convert_list_to_str(modules), user_input_env) == 0:
            ...
            try:
                logger.info('prometheus: updating static files for: %s', user_input_env)
            ...
            logger.info("prometheus: there is an existing configuration: %s", project_name)
            logger.info("prometheus: validating if endpoints have changed...")
...
def prometheus_validate_current_setup(prometheus_basefile, project, module, env):
    ...
    logger.debug("prometheus: final configured project name: %s", final_project_name)
    ...
    try:
        ...
        logger.debug("prometheus: no project config exists...")
        pass
    try:
        if final_project_name in (str(values)):
            logger.debug("prometheus: config entry already exists for: %s", project)
        else:
            logger.debug("prometheus: config entry does not exist for: %s", project)
            return 0
    except:
...
def prometheus_config(prometheus_fileloc, prj_name, env):
    ...
    alert_route = []
    alert_receiver = []
    logger.info("In prometheus_config function...")
    logger.info("prometheus: updating prometheus rules section...")
    ...
    logger.debug("prometheus: updating job section...")
...
def rules_setup(rules_dir, sample_file, project_name, env, project_name_without_env):
    ...
    logger.info("=" * 75)
    logger.info("prometheus: rules_setup function...")
...
def static_files_setup(prometheus_staticfiles_dir, staticfiles_sample_file, project_name, targets_to_monitor, env):
    #current_list_of_projects = []
    logger.info("=" * 75)
    logger.info("prometheus: static-files dir: %s", os.path.join(prometheus_staticfiles_dir, env))
    ...
    logger.info("prometheus: trying to delete: %s", project_name)
    # try:
    ...
    #     pass
    logger.info("prometheus: copying sample file from: %s to: %s", staticfiles_sample_file, full_filename)
    try:
    ...
    f.close()
    # todo: move to sample file format
    logger.info("prometheus: job name: %s", project_name)
    newdata = filedata.replace("USER_INPUT_PROJECT_NAME", project_name)
    ...
    #
    # except:
    #     logger.debug("prometheus: unable to delete file: %s", project_name)
    #     pass
...
def update_targets(prometheus_staticfiles_dir, project_name, end_point, modules, env):
    ...
    logger.info("prometheus: static-files location: %s", prometheus_staticfiles_dir)
    logger.info("prometheus: project_name: %s", project_name)
    logger.info("prometheus: currently configured endpoints: %s", end_point)
    ...
    try:
        with open(static_file, 'r') as stream:
            out = yaml.load(stream)
            logger.debug("prometheus:  output as json: %s", out)
        ...
        logger.info("prometheus: new target: %s", new_targets_string)
...
def update_prometheus(env, project_name, monitoring_config_file_path, monitoring_rules_dir, monitoring_static_file_dir):
    logger.info("=" * 75)
    try:
        logger.info("promethues: updating repo...")
...
def update_prometheus_rules_dir(prometheus_rules_dir):
    ...
    for rule_file in files:
        if "-updated.yaml" in rule_file:
            ...
            logger.debug("rule_file_name %s and rule_file %s ", rule_file_name, rule_file)
            try:
                if os.path.exists(os.path.join(prometheus_rules_dir, rule_file_name)):
                    logger.debug("removing  rule_file_name  %s", os.path.join(prometheus_rules_dir, rule_file_name))
            ...
            try:
                ...
                logger.debug("renaming %s to  %s", rule_file, rule_file_name)
            ...
            if os.path.exists(os.path.join(prometheus_rules_dir, rule_file)):
                logger.debug("removing  rule_file  %s", os.path.join(prometheus_rules_dir, rule_file))
...
def update_prometheus_staticfiles_dir(prometheus_staticfiles_dir):
    ...
    for static_file in files:
        if "-updated.yaml" in static_file:
            logger.info("updating staticfile %s", static_file)
            static_file_name = static_file.replace("-updated.yaml", "")
            logger.debug("static_file_name %s and static_file  %s", static_file_name, static_file)
            try:
                if os.path.exists(os.path.join(prometheus_staticfiles_dir, static_file_name)):
                    logger.debug("removing  static_file_name  %s", os.path.join(prometheus_staticfiles_dir, static_file_name))
            ...
            try:
                ...
                logger.debug("renaming %s to  %s", static_file, static_file_name)
            ...
            if os.path.exists(os.path.join(prometheus_staticfiles_dir, static_file)):
                logger.debug("removing  static_file  %s", os.path.join(prometheus_staticfiles_dir, static_file))
