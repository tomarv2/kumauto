...
def build_prometheus(config_yaml,
    ...
    logger.error("[prometheus] file location: %s", prometheus_fileloc)
    logger.error("[prometheus] rules dir: %s", prometheus_rules_dir)
    logger.error("[prometheus] static_files dir: %s", prometheus_staticfiles_dir)
    logger.error("[prometheus] verifying if config already exists...")
    ...
    try:
        if prometheus_validate_current_setup(prometheus_fileloc, project_name, convert_list_to_str(modules), user_input_env) == 0:
            try:
                logger.error('[prometheus] updating config for: %s', user_input_env)
                prometheus_config(prometheus_fileloc, project_name, user_input_env)
            except:
                logger.error("[prometheus] unable to update config...")
            try:
                logger.error('[prometheus] updating rules for: %s', user_input_env)
                logger.error('[prometheus] rules dir: %s', os.path.join(prometheus_rules_dir, user_input_env))
                logger.error("[prometheus] sample rule file :%s", monitoring_rules_sample_file)
                logger.error("[prometheus] project_name and user_input_env: %s, %s", project_name, user_input_env)
            ...
            logger.error("[prometheus] there is an existing configuration: %s", project_name)
            logger.error("[prometheus] validating if endpoints have changed...")
...
def prometheus_validate_current_setup(prometheus_basefile, project, module, env):
    logger.error("-" * 75)
    logger.error("[prometheus] validating if alert already exists...")
    logger.error("[prometheus] config file: %s", prometheus_basefile)
    logger.error("[prometheus] project name: %s", project)
    logger.error("[prometheus] module name: %s", module)
    logger.error("[prometheus] env name: %s", env)
...
def prometheus_config(prometheus_fileloc, prj_name, env):
    ...
    alert_route = []
    alert_receiver = []
    logger.error("In prometheus_config function...")
    logger.error("[prometheus] updating prometheus rules section...")
    logger.error("[prometheus] taking backup of file...")
...
def rules_setup(rules_dir, sample_file, project_name, env, project_name_without_env):
    ...
    logger.error("-" * 75)
    logger.error("[prometheus] rules_setup function...")
    logger.error("[prometheus] rules dir: %s", rules_dir)
    logger.error("[prometheus] rules sample file: %s", sample_file)
    logger.error("[prometheus] rules project_name: %s", project_name)
    try:
        ...
        raise SystemExit
    try:
        logger.error("[prometheus] copy sample file: %s to %s", sample_file, rules_file_name)
...
def static_files_setup(prometheus_staticfiles_dir, staticfiles_sample_file, project_name, targets_to_monitor, env):
    #current_list_of_projects = []
    logger.error("-" * 75)
    logger.error("[prometheus] static-files dir: %s", os.path.join(prometheus_staticfiles_dir, env))
    logger.error("[prometheus] static-files sample file: %s", staticfiles_sample_file)
    full_filename = os.path.join(prometheus_staticfiles_dir, env, project_name) + '-blackbox.yaml' + "-updated.yaml"
    logger.error("[prometheus] static-file name: %s", full_filename)
    ...
    #     pass
    logger.error("[prometheus] copying sample file from: %s to: %s", staticfiles_sample_file, full_filename)
    try:
        logger.error("[prometheus] trying...")
    ...
    #     current_list_of_projects.extend(filenames)
    #     break
    logger.error("[prometheus] trying to open static file...")
    f = open(full_filename, 'r')
    filedata = f.read()
    f.close()
    # todo: move to sample file format
    logger.error("[prometheus] job name: %s", project_name)
    newdata = filedata.replace("USER_INPUT_PROJECT_NAME", project_name)
...
def update_targets(prometheus_staticfiles_dir, project_name, end_point, modules, env):
    ...
    logger.error("[prometheus] static-files location: %s", prometheus_staticfiles_dir)
    logger.error("[prometheus] project_name: %s", project_name)
    logger.error("[prometheus] currently configured endpoints: %s", end_point)
    modules = ([y for x in modules for y in x])
    modules = ''.join(modules)
    logger.error("[prometheus] modules: %s", modules)
    logger.error("[prometheus] static file: %s", static_file)
    project_filename = project_name + '-' + modules
    logger.error("[prometheus] job name: %s", project_filename)
    try:
